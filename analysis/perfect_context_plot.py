...
# Create grouped bar chart organized by model family with 3 metrics
...
for idx, metric in enumerate(metrics):
    ...
    ax.set_ylabel(metric_titles[metric], fontsize=12, fontweight='bold')
    ax.set_title(f'{metric_titles[metric]}', fontsize=14, fontweight='bold', pad=15)
    ax.set_xticks(x)
    ax.set_xticklabels(sorted_model_names, rotation=60, ha='right', fontsize=8)
    ax.set_ylim(0, 1.05)
    ax.grid(axis='y', alpha=0.3, linestyle='--')

fig.suptitle('Perfect Context: Performance Metrics by Model Family',
             fontsize=16, fontweight='bold', y=1.02)

plt.tight_layout()
plt.savefig('plots/perfect_context_by_family.png', dpi=300, bbox_inches='tight')
print("\n✓ Plot saved to: plots/perfect_context_by_family.png")
plt.show()
